...
def objective(trial: optuna.Trial):
    ...
    dict_args = {
        "gpus": gpu_id,
        "name": env_name,
        "gloss_dataset_name": "SREF_enhanced_embeddings",
        "max_epochs": 10,
        "shuffle": True
    }
    ...
    dict_args["log_every_n_steps"] = int(500 * 128 / batch_size)
    if batch_size == 256:
    ...
    if context_dataset_name == "":
        ...
        if (gloss_projection_head_name == "Identity") and (context_projection_head_name in ("COPY", "SHARED", "Identity")):
            return 0.0
        elif gloss_projection_head_name != context_projection_head_name:
            return 0.0
    ...
    if gloss_projection_head_name == "Identity":
        ...
        if gloss_projection_head_name == "NormRestrictedShift":
            cfg_gloss_projection_head["max_l2_norm_ratio"] = trial.suggest_loguniform("max_l2_norm_ratio", low=0.01, high=0.5)
    ...
    if context_projection_head_name in ("COPY", "SHARED", "Identity"):
        ...
        if context_projection_head_name == "NormRestrictedShift":
            cfg_context_projection_head["max_l2_norm_ratio"] = trial.suggest_loguniform("max_l2_norm_ratio", low=0.01, high=0.5)
